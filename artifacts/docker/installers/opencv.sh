...
cmake -D CMAKE_BUILD_TYPE=Release -D CMAKE_INSTALL_PREFIX=$INSTALL_PREFIX \
  ...
  -D WITH_VTK=ON -D WITH_CUDA=ON -D OPENCV_FORCE_3RDPARTY_BUILD=ON -D WITH_PROTOBUF=OFF -D BUILD_PROTOBUF=OFF \
  -D BUILD_LIST=calib3d,highgui,imgcodecs,flann,features2d,cudev,cudafeatures2d,cudafilters,gapi,line_descriptor ..
make -j$(($(nproc)-1))
make install
popd
popd
